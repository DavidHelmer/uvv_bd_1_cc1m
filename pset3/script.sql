/* Conectando ao sistema Postgres */
psql -U postgres -d postgres

/* Conectando ao banco de dados uvv já existente */
\echo
\echo Conectando ao banco de dados:
\c "dbname=uvv user=david password=123"

-- Ajusta o SEARCH_PATH da conexão atual, por via das dúvidas:
SET SEARCH_PATH TO elmasri, "$user", public;

/* Classificação
Criação da tabela classificação que serve para armazenar a hierarquia
de classificações de produtos da empresa, e os demais objetos relacionados */
...
/* Inserção dos dados
Faz a inserção de dados na tabela classificação, respeitando
a ordem devida de classificações "pai" e "filhos".
*/
